...
class MaxHeap(Generic[T]):
    ...
    def get_max(self) -> T:
        ...
        :complexity: Best O(1) + O(1) + OCompare when the element is greater or equal to one of its children (cannot be smaller), which means OCompare
                     Worst O(1) + O(1) + O(logN)*O(1)*OCompare when the element sinks all the way to the bottom, which means O(logN)*OCompare
        """
        if self.length == 0:
            raise IndexError
