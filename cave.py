""" 
Caves. Players visit them to mine materials.

Caves have a name and a certain quantity of a specific material. Supports random generation of a cave. Defines a unique cave as having
a unique name, so caves can have duplicate materials as long as they have different names. 
"""
...
class Cave:
    """
        Place where players can mine Materials

        Attributes:
        * name: Used to identify the cave
        * material: The material stored within this cave. Each cave houses exactly 1 material
        * quantity: The original amount of the material which is mineable in the cave.
        * mined_quantity: Amount of material mined(limited by hunger points)
        * remaining_quantity: Remaining material after a player mined from the cave.
        * is_visited: Flag that indicates that a cave has been visited. Helps prepare caves for next day.
    ...
    def __init__(self, name: str, material: Material, quantity: float=0) -> None:
        """
            ...
            :param arg1: name of cave
            :param arg2: material object
            :param arg3: quantity of material
        ...
        self.mined_quantity = None
        self.remaining_quantity = None
        self.is_visited = False
    
    def add_quantity(self, amount: float) -> None:
        """
            Increases quantity of the material in the cave by <amount> at the end of each day.
            
            :param arg1: amount of material quantity to be added
    ...
    def remove_quantity(self, amount: float) -> None:
        """
            Decreases quantity of the material in the cave by <amount> at the end of each day.

            :param arg1: amount of material quantity to be removed
    ...
    def get_quantity(self) -> float:
        """
            Returns current quantity of the material in the cave

            :param: None

            :pre: None

            :return: current quantity of material
    ...
    def __str__(self) -> str:
        """
            Returns string representation which includes cave name and current quantity of material
            ...
            :complexity: Best/Worst O(1)
        """
        return f"{self.name} consist of {self.get_quantity()} {self.material}"

    def __eq__(self, other: Cave) -> bool:
        """
            Returns True if Caves are equal. Two Caves are different if they have distinct names.

            :param arg1: cave object

            :pre: None

            :return: True if current (self) cave object name is equal to other cave object name, False otherwise.
    ...
    def random_cave(cls, material_list: list[Material]) -> Cave:
        """
            Generates and returns a random cave
            
            :param arg1: list of material objects

            :pre: None

            :return: random cave object

            :complexity: Best/Worst O(1)
        """
        random_name = RandomGen.random_choice(CAVE_NAMES) # gets random name from list of random names provided
        random_material = RandomGen.random_choice(material_list) # gets random material from material_list
        random_quantity = RandomGen.randint(1, 10) # arbitrary generation of quantity

        return cls(random_name, random_material, random_quantity)
